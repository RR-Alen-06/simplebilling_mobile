...
class _BillingScreenState extends ConsumerState<BillingScreen> {
    ...
    BarcodeScannerModal.show(
      ...
      onProductScanned: (product) {
        ref
            ...
            .addItem(product.name, product.price, 1, productId: product.id);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
  ...
  Future<void> _handleCheckout() async {
    ...
    final loyaltyDiscount = cart.calculateLoyaltyDiscount(
      settings.loyalty,
      loyaltyRules,
    );

    final subtotalAfterDiscount =
        (cart.subtotal - cart.manualDiscount - loyaltyDiscount).clamp(
          0.0,
          double.infinity,
        );
    final gstAmount = cart.calculateGst(
      settings.billing,
      subtotalAfterDiscount,
    );
    final roundingResult = cart.getRoundingResult(
      loyaltyDiscount,
      gstAmount: gstAmount,
    );
    ...
    double finalUpi = upi;
    if (cash == 0 && upi == 0) {
      final remaining = (roundingResult.roundedTotal - advanceUsed).clamp(
        0.0,
        double.infinity,
      );
      finalCash = remaining;
    ...
    if (settings.loyalty.enabled &&
        (finalCash + finalUpi + advanceUsed) >=
            roundingResult.roundedTotal - 0.01) {
      pointsEarned = await ApiRepository.calculateLoyaltyPointsEarned(
        roundingResult.roundedTotal,
      );
    }
    ...
    if (newBill == null) {
      ...
      final offlineBillNumber =
          'OFFLINE-BILL-${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(
          children: [
            ...
            Text(
              bill.billNumber.startsWith('OFFLINE')
                  ? 'Bill Queued (Offline)'
                  : 'Bill Generated!',
            ),
          ],
          ...
          children: [
            ...
            if (bill.loyaltyPointsEarned > 0)
              Padding(
                ...
                child: Text(
                  'Points Earned: +${bill.loyaltyPointsEarned.toStringAsFixed(0)} pts',
                  style: const TextStyle(
                    color: Colors.orange,
                      ...
                      return GridView.builder(
                        ...
                        itemBuilder: (ctx, i) {
                          ...
                          return InkWell(
                            onTap: () => ref
                                .read(cartProvider.notifier)
                                .addItem(p.name, p.price, 1, productId: p.id),
                            child: Card(
                              elevation: 0.5,
                              shape: RoundedRectangleBorder(
                      ...
                      children: [
                        ...
                        if (cart.manualDiscount > 0 || loyaltyDiscount > 0) ...[
                          ...
                          Row(
                            ...
                            children: [
                              const Text(
                                'Discount (Manual + Loyalty):',
                                style: TextStyle(color: AppColors.error),
                              ),
                              Text(
                                '- ${Formatters.currency(cart.manualDiscount + loyaltyDiscount)}',
                                style: const TextStyle(
                                  color: AppColors.error,
                                  fontWeight: FontWeight.w600,
                                ),
                        ...
                        if (roundingResult.roundingAdjustment != 0) ...[
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              Text(
                                Formatters.currency(
                                  roundingResult.roundingAdjustment,
                                ),
                              ),
                            ],
                          ),
